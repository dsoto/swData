...
# read in index
# pull in preload and drag values
# and place in index dictionary
index = rx.readDataFileArray(fileIndex)
index['preload'] = [('%02d' % float(p)) for p in index['preload']]
index['drag'] = [('%02d' % float(p)) for p in index['drag']]
...
for i in range(numFiles):
    ...
    fileOutName = prefix + 'p%s-d%s.data' % (index[i][2], index[i][3])
