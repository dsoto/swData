...
%contactPointDetect = 0; % use minimum
contactPointDetect = 1; % use threshold
...
if (analyze)
	...
	% use minimum to find point of contact
	if (contactPointDetect == 0)
  [lateralForceAtContactMicroNewton, indexContact] = ...
  		min(lateralForceMicroNewton(1:indexMaxPreload));
	end
	
	% use threshold to determine point of contact
	if (contactPointDetect == 1)
		threshold = 7.0 * noiseFloorLateral;
		indexStartSearch = 1;
...
legend('Shear','Normal');			
title({'20080910_sws6_sb - Cantilever 629a03',titleString}, ...
		'Interpreter','None');
